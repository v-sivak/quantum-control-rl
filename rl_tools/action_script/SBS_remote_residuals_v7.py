...
cavity_phase = [2.5, -2.3, -2.3, -2.3] # gg, ge, eg, ee
Kerr_drive_amp = [0.0, 0.0, 0.0, 0.0] # gg, ge, eg, ee
...
# script['alpha'] = [[[np.real(a), np.imag(a)] for a in alpha_complex]] # shape=[1,4,2]
script['phi'] = [[[p,a] for (p,a) in zip(qb_phase, qb_angle)]] # shape=[1,4,2]
script['echo_pulse'] = [[[p,a] for (p,a) in zip(echo_phase, echo_angle)]] # shape=[1,4,2]

script['cavity_phase'] = [cavity_phase] # shape=[1,4]
script['Kerr_drive_amp'] = [Kerr_drive_amp] # shape=[1,4]
script['qb_detune'] = [[qb_detune]] # shape=[1,1]
script['qb_drag'] = [[qb_drag]] # shape=[1,1]
script['cavity_detune'] = [[cavity_detune]] # shape=[1,1]
script['alpha_correction'] = [[alpha_correction]*3] # shape=[1,3,2]
script['reset_pulse'] = [reset_pulse] # shape=[1,2]
script['init_phase'] = [[init_phase]] # shape=[1,1]
