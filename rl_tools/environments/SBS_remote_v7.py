...
class QuantumCircuit(OscillatorQubit, TFEnvironmentQuantumControl):
    ...
    def _control_circuit_spec(self):
        spec = {'beta'  : specs.TensorSpec(shape=[4,2], dtype=tf.float32), 
                # 'alpha' : specs.TensorSpec(shape=[4,2], dtype=tf.float32), 
                'phi'   : specs.TensorSpec(shape=[4,2], dtype=tf.float32),
                'echo_pulse'  : specs.TensorSpec(shape=[3,2], dtype=tf.float32),
                'cavity_phase' : specs.TensorSpec(shape=[4], dtype=tf.float32),
                'Kerr_drive_amp' : specs.TensorSpec(shape=[4], dtype=tf.float32),
                'alpha_correction' : specs.TensorSpec(shape=[3,2], dtype=tf.float32),
                ...
                'init_phase' : specs.TensorSpec(shape=[1], dtype=tf.float32)
                }
        return spec
