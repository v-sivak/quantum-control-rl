...
class QuantumCircuit(Oscillator, GKP):
    ...
    needed to evacuate entropy) of the small-Big-small protocol.
    ...
    def _quantum_circuit_spec(self):
        spec = {'beta' : specs.TensorSpec(shape=[1,2], dtype=tf.float32), 
                'eps1' : specs.TensorSpec(shape=[1,2], dtype=tf.float32),
                'eps2' : specs.TensorSpec(shape=[1,2], dtype=tf.float32),
    ...
    def _quantum_circuit(self, psi, action):
        ...
        Args:
            ...
            action (dict, 'beta' : Tensor([batch_size,1,2], tf.float32),
                          'eps1' : Tensor([batch_size,1,2], tf.float32,
                          'eps2' : Tensor([batch_size,1,2], tf.float32,
        ...
        T['+e1'] = self.translate(epsilon/2.0)
        T['-e1'] = tf.linalg.adjoint(T['+e'])
        T['+e2'] = self.translate(epsilon/2.0)
        T['-e2'] = tf.linalg.adjoint(T['+e'])
