...
class sbs_feedback_reset_wigner_1D_Mixer(FPGAExperiment, GKP):
    ...
    def sequence(self):
        ...
        tomo_phase = (self.cavity_phase + np.pi/2.0 + self.phase) * 2 * self.reps

        # map odd parity to 'e'
        with system.wigner_tomography_1D(*self.disp_range, result_name='g_m2',
                                      invert_axis=False, phase=tomo_phase):
            readout(g_m0='se')
            exp()
        ...
        with system.wigner_tomography_1D(*self.disp_range, result_name='e_m2',
                                      invert_axis=True, phase=tomo_phase):
            readout(e_m0='se')
            exp()
            self.reset_feedback_with_echo(self.echo_delay, self.final_delay,
