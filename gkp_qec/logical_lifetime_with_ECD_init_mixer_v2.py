...
class logical_lifetime_with_ECD_init_mixer_v2(FPGAExperiment):
    ...
    def sequence(self):
        ...
        def reset(phase_reg, phase_g_reg, phase_e_reg):
            gkp.reset_feedback_with_phase_update_and_Kerr_drive(
                    phase_reg, phase_g_reg, phase_e_reg, detune=qb_detune, drag=qb_drag,
                    Kerr_g_amp=Kerr_g_amp, Kerr_e_amp=Kerr_e_amp)
    ...
    def process_data(self):
        ...
        for s in ['X','Y','Z']:
            init_state_g = self.results['m0_'+s].multithresh()[0]
            postselected = self.results['m2_'+s].postselect(init_state_g, [1])[0]

            logical_pauli = (1 - 2*postselected.thresh_mean().data) * flip_mask[s]
